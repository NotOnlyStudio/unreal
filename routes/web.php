...
Route::post('tinkoff/getPayments', function (Request $request) {

    $data = $request->all();

    $paymentUpdateUserAction = new PaymentUpdateUserAction();
    $paymentUpdateAction = new PaymentUpdateAction();

    $id = $data['Data']['ID'];
    \Illuminate\Support\Facades\Storage::put('test.json', $id);

    $payment = Payment::query()->where([['payment_key', $id], ['status', '!=', true]])->first();
//    return $payment;
    if ($data['Status'] == 'CONFIRMED' and $payment) {
        $paymentUpdateAction->handle($id);
        $paymentUpdateUserAction->handle($payment->price, $id);
    }
...
Route::get("/", function () {
    $blog = \App\Models\Blog::where("id", '!=', 0)->where("moderation", "=", true)->with(["user" => function ($query) {
