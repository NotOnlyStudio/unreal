<template>
  <div :class="['mode__selector', mode ? 'light' : 'dark']">
    <svg @click="toggleMode" class="sun"
    ...
    <input type="checkbox" class="mode" id="mode" />
    <label @click="toggleMode" for="mode"></label>
    <svg @click="toggleMode" class="moon"
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
      version="1.1"
      id="Capa_1"
    ...
    <audio src="/assets/switch.mp3" class="d-none" volume="30" id="switchSound"></audio>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      mode: this.modeValue == true ? true : false,
    };
  },
  methods: {
    toggleMode() {
        this.mode = !this.mode
        let app = document.querySelector("#app").classList
        $cookies.set("theme_mode", this.mode)
        let audio = document.querySelector("#switchSound")
        audio.volume = 0.2
        audio.play();
        app.toggle("theme-dark")
        app.toggle("theme-light")
    },
  },
};
</script>