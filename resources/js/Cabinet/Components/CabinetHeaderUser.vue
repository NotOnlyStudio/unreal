<template>
  <div>
    <transition>
      <upload-form
          @uploadedPhoto="changeUserPhoto"
          @keydown.esc="uploadAvatar = false"
    ...
    <div :class="[columns && columns != 0 ? 'double-column' : '','d-flex content-right']">
      <div class="container">
        ...
        <div class="d-flex flex-column">
          <div class="profile">
            <div class="profile__main">
              ...
              <p class="name" :title="user ? user.name : 'None'">{{
        ...
        <b-alert show variant="light row">
          <div class="d-flex models-counts mt-4 align-items-center col-4">
            <b-button variant="bordered sm mx-4" href="/basket" >Buy models</b-button>
          </div>
          <div class="d-flex models-counts mt-4 align-items-center col-4">
            <b-button variant="bordered sm mx-4 start-btn" @click.prevent="startChatting(user.id)">Start chatting {{  }}</b-button>
          </div>
        </b-alert>
        <vnav :user-no-owner="userNoOwner"></vnav>
      </div>
    </div>
    ...
    startChatting(num)
    {
      $.ajax({
        ...
        success: function(data){
          window.location.href = '/cabinet/messages?id='+data
        }
      });
    }
